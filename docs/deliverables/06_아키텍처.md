...
계층(L0~L5) 관점과 별개로, 시스템을 **책임 단위의 Core 모듈**로 묶으면
탐지·대응 파이프라인의 데이터 흐름이 더 직관적으로 드러난다.
Core 1 → 2 → 3 으로 신호가 흐르고, Core 3 은 Core 1 에 격리/종료 명령을
되돌려주며(피드백), Core 4 가 결과를 가시화한다. Support 는 전 Core 의
배포·기동을 담당하는 토대 계층이다.

| Core | 이름 | 기능군 (F) | 주요 컴포넌트 |
|------|------|-----------|---------------|
| **Core 1** | 커널 감시·차단 | F1 | `RansomGuard.sys` (minifilter IRP 콜백), 격리 PID 비트맵[256], 통신 포트 `\RansomGuardPort` (50ms) |
| **Core 2** | 행위 탐지 | F2 | `minifilter_bridge`, Canary, Mass-IO/FS 버스트, Cmdline 룰, Process 트리 |
| **Core 3** | 판단·능동 대응 | F3 · F4 | `ScoringEngine` (120s 윈도우), `EventStore`, `Responder` (OFF/QUARANTINE/KILL · NEVER_KILL), `actor_trust`, `tamper` 자가방어 |
| **Core 4** | 가시화·보고 | F5 | Flask Dashboard, `IncidentReporter`, 로깅/알림 |
| **Support** | 설치·운영 | F6 | bootstrap, driver build, service/watchdog |

> 데이터 흐름:  Core 1 →(`RG_EVENT` 50ms)→ Core 2 →(`Signal(weight)`)→ Core 3
> →(`Quarantine/Kill` 명령, 피드백)→ Core 1, Core 3 →(이벤트/리포트)→ Core 4 →(대시보드/알림)→ 관리자·사용자

![Core 모듈 아키텍처](images/07_core_architecture.png)
