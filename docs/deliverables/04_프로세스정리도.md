...
| K5 | `RgCreateProcessNotify` (v2) | `PsSetCreateProcessNotifyRoutineEx` — 프로세스 생성/종료 동기 감지, cmdline 을 `Extra` 에 채워 emit |
| K6 | `RgCmRegistryCallback` (v2) | `CmRegisterCallbackEx` — Defender/Run/RansomGuard 등 watch 키 변경 시 `RgEventRegistry` emit |
| K7 | `RgObPreOperation` (v2) | `ObRegisterCallbacks` — 보호 PID 핸들 open 시 위험 권한 strip, `RgEventTamperBlocked` emit |
| K8 | `RgSendEvent` | `RG_EVENT` non-paged pool 할당 → `FltSendMessage` (50ms 타임아웃) |
...
| D7 | `ProcessKernelDetector` (v2) | bridge `PROCESS_START` 콜백 → cmdline 룰 적용 (WMI 미사용, 지연 0ms) |
| D8 | `RegistryKernelDetector` (v2) | bridge `REGISTRY` 콜백 → 키 경로를 라벨 시그널로 매핑 |
...
| 단계 | 함수 | 처리 |
|------|------|------|
| R1 | `_dispatch(sig, score, level)` | HIGH/CRITICAL + PID + `_is_confident()` → `_respond_to_pid`, 아니면 `_record_observed`(무대응). 점수 기반 전체 PID 스윕은 폐지 |
...
| `cmdline_*` (커널 출처) | `process_kernel.py` (v2) | 35~70 | HIGH/CRITICAL 직행 |
| `ransomguard_service_tamper` | `registry_kernel.py` (v2) | 80 | CRITICAL |
| `defender_*_tamper` | `registry_kernel.py` (v2) | 70 | CRITICAL |
| `ifeo_hijack` | `registry_kernel.py` (v2) | 60 | HIGH |
| `tamper_blocked` | `minifilter_bridge.py` (v2) | 50 | HIGH |
