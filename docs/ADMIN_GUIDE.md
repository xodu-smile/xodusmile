...
### 브라우저(대시보드 UI)에서 토큰 사용

대시보드 헤더의 **🔑 토큰** 버튼으로 토큰을 입력하면 이후 모든 API 호출에
`X-API-Key` 헤더가 자동 첨부됩니다. 토큰은 해당 탭의 `sessionStorage` 에만
보관되며(탭을 닫으면 사라짐) 디스크/`localStorage` 에 남지 않습니다.
토큰이 필요한 호출이 401 을 받으면 버튼이 **"🔑 인증 필요"** 경고 상태로
바뀝니다.
